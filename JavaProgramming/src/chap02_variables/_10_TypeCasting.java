...
public class _10_TypeCasting {
	...
	public static void main(String[] args) {
		...
		// 1. 자동형변환1: btye 크가가 작은 타입에서 큰 타입으로 자동형변환이 일어난다.
		short sNum1 = 10;
		...
		// long은 8btye고 float는 4byte로 long이 byte크기는 크지만 float가 더 세밀한 표현범위를 갖고 있기 때문에
		// lNum1이 float 타입으로 변환된다.
		float fNum1 = lNum1;
		
		// 3. 강제형변환1:byte크기가 큰 타입에서 작은 타입으로 변환할때는 타입을 명시해줘야 한다.
		int iNum2 = 60;
		
		// int 타입인 iNum2가 short 타입보다 byte 크기가 크기 때문에 강제로 short 타입으로 변한다.
		...
		// 6. 문자열 결합 연산
		// 문자열 결합 연산에서는 모든 타입이 문자여로 변환된 후 문자열과 결합된다.
		String str = "bitcamp";
		int classRoom = 502;
